...
makeData <- function(i){ # by default, dolby = T
  ...
  # gender
  themodel <- includeEffects(themodel, name = "dislike", altX, egoX, sameX, sameXRecip, interaction1= "gender")
  # ethnicity
...
# when there is no convergence problem
# we can go with the loop as we did for the base model for friendship network
# but by doing it one by one, we find the class with convergence problem 
...
makeData <- function(i){ # by default, dolby = T
  ...
  # Algorithms:
  algo0 <- sienaAlgorithmCreate(projname = paste('model1_', i, sep=""), n3=5000,seed=1234,firstg = 0.02)
  # "continue the estimation, using prevAns,with nsub=1; a high value for n3, e.g., 3000 or 5000; and a high value for n2start."
  # The default n2s is 2.52×(p+7), where p is the number of estimated parameters, divided
  # by nbrNodes, if multiple processes are used.
  #  For a ‘high value’, start with a value that is about twice as large(*2)
  ...
  algo1 <- sienaAlgorithmCreate(projname = paste('model1_', i, sep=""),
                                nsub=1, n2start = n2s, n3=5000,seed=1234,firstg = 0.02)
...
max(sapply(resultsList0, function(x){x$tconv.max})) # ~0.15
...
(resultsList0[[1]]$sd)^2 <0.1  # true for same gender reciprocity
...
GOFP # p of the triadcensus of class 67100 is 0 
...
meta_dl_1 <- siena08(resultsList0,  projname = "meta_dl")
meta_dl_1
save(meta_dl_1, file='meta_dl_1.Rdata')
# Look at the order of the list elements of which meta_fr_1 is composed:
names(meta_dl_1)
meta.table(meta_dl_1, filename="meta08_1.2.tex", option=2)
meta.table(meta_dl_1, filename="meta08_1.tex", option=1)
summary(meta_dl_1)


# aree the SE's RANDOM?
plot(meta_dl_1, which = 1:length(meta_dl_1$theta),
     useBound=TRUE)

neff <- sum(resultsList0[[1]]$effects$include)
parameters.08 <- t(sapply(1:neff, function(i){c(meta_dl_1[[i]]$mu.ml,
                                                meta_dl_1[[i]]$mu.ml.se, meta_dl_1[[i]]$mu.confint,
                                                meta_dl_1[[i]]$sigma.ml, meta_dl_1[[i]]$sigma.confint,
                                                meta_dl_1[[i]]$n1)}))
...
efnames <- names(meta_dl_1)[1:neff]

# Extract heterogeneity tests from siena08 output;
# these are called Q in Snijders & Baerveldt (2003):
hetero.08 <- t(sapply(1:neff, function(i){c(meta_dl_1[[i]]$Qstat,
                                            meta_dl_1[[i]]$n1-1, meta_dl_1[[i]]$pttilde)}))
...
Overalls.08 <- t(sapply(1:neff, function(i){c(meta_dl_1[[i]]$Tsq,
                                              meta_dl_1[[i]]$n1-1, meta_dl_1[[i]]$pTsq)}))
...
Fishers <- t(sapply(1:neff,
                    function(i){c(meta_dl_1[[i]]$cjplus, meta_dl_1[[i]]$cjminus, meta_dl_1[[i]]$cjplusp, meta_dl_1[[i]]$cjminusp, 2*meta_dl_1[[i]]$n1 )}))
