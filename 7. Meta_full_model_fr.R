...
makeData <- function(i){ # by default, dolby = T
  ...
  themodel <- includeEffects(themodel,name='friendship',
                             X, interaction1='gp')
  # gp reciprocity
  themodel <- includeEffects(themodel,name='friendship',
...
GOFP # gof1.triads of the first and last classroom is 0.
...
meta_fr_full <- siena08(resultsList0,  projname = "meta_fr_full")
meta_fr_full
save(meta_fr_full, file='meta_fr_full.Rdata')
# Look at the order of the list elements of which meta_fr_1 is composed:
names(meta_fr_full)
meta.table(meta_fr_full, filename="meta08_full.2.tex", option=2)
meta.table(meta_fr_full, filename="meta08_full.tex", option=1)
?meta.table

summary(meta_fr_full)


# aree the SE's RANDOM?
plot(meta_fr_full, which = 1:length(meta_fr_full$theta),
     useBound=TRUE)

neff <- sum(resultsList0[[1]]$effects$include)
parameters.08 <- t(sapply(1:neff, function(i){c(meta_fr_full[[i]]$mu.ml,
                                                meta_fr_full[[i]]$mu.ml.se, meta_fr_full[[i]]$mu.confint,
                                                meta_fr_full[[i]]$sigma.ml, meta_fr_full[[i]]$sigma.confint,
                                                meta_fr_full[[i]]$n1)}))
...
save(parameters.08,file = 'Meta.Parfull.Rdata')



# Construct the names for the effects:
efnames <- names(meta_fr_full)[1:neff]

# Extract heterogeneity tests from siena08 output;
# these are called Q in Snijders & Baerveldt (2003):
hetero.08 <- t(sapply(1:neff, function(i){c(meta_fr_full[[i]]$Qstat,
                                            meta_fr_full[[i]]$n1-1, meta_fr_full[[i]]$pttilde)}))
...
Overalls.08 <- t(sapply(1:neff, function(i){c(meta_fr_full[[i]]$Tsq,
                                              meta_fr_full[[i]]$n1-1, meta_fr_full[[i]]$pTsq)}))
...
Fishers <- t(sapply(1:neff,
                    function(i){c(meta_fr_full[[i]]$cjplus, meta_fr_full[[i]]$cjminus, meta_fr_full[[i]]$cjplusp, meta_fr_full[[i]]$cjminusp, 2*meta_fr_full[[i]]$n1 )}))
colnames(Fishers) <- c('Fplus', 'Fminus', 'pplus', 'pminus', 'df')
rownames(Fishers) <- efnames
round(Fishers,3)

save(Fishers, file = 'Fishersfull.Rdata')
